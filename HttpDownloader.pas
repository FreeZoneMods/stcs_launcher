...
{ FZDownloaderThread }
constructor FZDownloaderThread.Create(dll_path:string);
var
  tid:THandle;
begin
  inherited Create();
  ...
  _dll_handle := LoadLibrary(PChar(dll_path));
  if _dll_handle = 0 then begin
    FZLogMgr.Get.Write('Cannot load '+dll_path, FZ_LOG_ERROR);
    _good := false;
    exit;
  end;
  ...
  _thread_active:=true;
  CreateThread(nil, 0, @DownloaderThreadBodyWrapper, self, 0, tid);
end;
